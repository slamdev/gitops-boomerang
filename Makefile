build:
	go build -o bin/gitops-boomerang ./cmd/boomerang

run: build
	./bin/gitops-boomerang throw
...
untag:
	...
	curl --request DELETE --header "Authorization: token ${GITHUB_TOKEN}" "https://api.github.com/repos/slamdev/gitops-boomerang/releases/:release_id/$(TAG)"
